        ...
        <template v-if="renderType === 'normal'">
            <template v-if="column.tooltip">
                <Tooltip transfer :content="row[column.key]" :theme="tableRoot.tooltipTheme" :disabled="!showTooltip && !tooltipShow" :max-width="300" class="ivu-table-cell-tooltip" @on-popper-show="handleTooltipShow" @on-popper-hide="handleTooltipHide">
                    <span v-bind:title="row[column.key]" ref="content" @mouseenter="handleTooltipIn" @mouseleave="handleTooltipOut" class="ivu-table-cell-tooltip-content">{{ row[column.key] }}</span>
                </Tooltip>
            </template>
            <span v-bind:title="row[column.key]" v-else>{{row[column.key]}}</span>
        </template>
            ...
            classes () {
                return [
                    `${this.prefixCls}-cell`,
                    {
                        ...
                        [`${this.prefixCls}-cell-with-btn`]: this.renderType === 'slot' && this.column.type == 'btn',
                        [`${this.prefixCls}-custom-btn-fa`]: this.renderType === 'slot' && this.column.type == 'btn',
                    }
                ];
            },
            expandCls () {
